...
let whereIsHere = ()=> {
  return isAppstore() ? "appstore" : isGameCenter() ? "gamecenter" : "browser";
};

let isVersionLt300 = ()=> {
  let versionCode = getCookie("vvc_app_version") || getCookie("app_version");
  let versionCodeNumber = parseInt(versionCode) || 0;
  return versionCodeNumber < 300;// 2.5版本对应值为300
};

let getAppstoreVersion = ()=> {
  let versionCode = getCookie("vvc_app_version") || getCookie("app_version");
  return parseInt(versionCode) || 0;
};
